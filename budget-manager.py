...
tab1_layout = [
    [sg.T("Upload CSV:", font=("Any", 13, "bold"), pad=((3, 0), (10, 0)))],
    [
        sg.T("Table:"),
    ...
    [
        sg.T("Amount:"),
        sg.InputText(size=(10, 1), key="expAmount", do_not_clear=False),
        sg.T("Category:"),
        sg.Combo(
            ["", "Shopping", "Groceries", "Food & Drink", "Travel", "Personal", "Bill"],
            key="expCat",
            default_value="",
        ),
        sg.T("Date:"),
        sg.CalendarButton(
            "Calendar", button_color=("black", "light gray"), key="expDate"
        ),
    ],
    [sg.T("Description:"), sg.InputText(key="expDescription", do_not_clear=False)],
    ...
    [
        ...
        sg.T("Date:"),
        sg.CalendarButton(
            "Calendar", button_color=("black", "light gray"), key="incDate"
        ),
    ],
...
def viewTable(tableName, year, month="", category=""):
    ...
    tableLayout = [[sg.Table(values=data, headings=headers)]]
...
def viewStats(year, nameMonth="", month=""):
    ...
    if month:
        ...
        monthSpent = round(
            expdf.loc[expdf.date.str.startswith(f"{month}"), "amount"].sum(), 2
        )
        monthIncome = round(
            incdf.loc[incdf.date.str.startswith(f"{month}"), "amount"].sum(), 2
        )
        ...
        g = expdf.loc[
            (expdf["category"] == "Groceries")
            & (expdf.date.str.startswith(f"{month}")),
            "amount",
        ].sum()
        t = expdf.loc[
            (expdf["category"] == "Travel") & (expdf.date.str.startswith(f"{month}")),
            "amount",
        ].sum()
        s = expdf.loc[
            (expdf["category"] == "Shopping") & (expdf.date.str.startswith(f"{month}")),
            "amount",
        ].sum()
        f = expdf.loc[
            (expdf["category"] == "Food & Drink")
            & (expdf.date.str.startswith(f"{month}")),
            "amount",
        ].sum()
        p = expdf.loc[
            (expdf["category"] == "Personal") & (expdf.date.str.startswith(f"{month}")),
            "amount",
        ].sum()
        b = expdf.loc[
            (expdf["category"] == "Bill") & (expdf.date.str.startswith(f"{month}")),
            "amount",
        ].sum()
        ...
        def draw_figure(canvas, loc=(0, 0)):
            ...
            patches, text = plt.pie(
                amounts,
                startangle=90,
                colors=["yellow", "purple", "red", "blue", "orange", "green"],
            )
            legLabels = [
                "{0} - $ {1:1.2f}".format(i, j) for i, j in zip(labels, amounts)
            ]
            plt.legend(
                patches,
                legLabels,
                bbox_to_anchor=(-0.625, 1.17),
                loc="upper left",
                fontsize=8,
            )
        ...
        statsLayout = [
            [sg.T(f"Summary for {nameMonth} {year}:", font=("Any", 13, "bold"))],
    ...
    if not month:
        ...
        # layout of summary stats window
        statsLayout = [
            [sg.T(f"Summary for {year}:", font=("Any", 13, "bold"))],
